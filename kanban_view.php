...
echo '<div class="container py-4">
    <h2 style="text-align: center;margin-bottom: 20px;">Organisez vos tâches</h2>
    ...
    if (isset($tasks[$category])) {
        foreach ($tasks[$category] as $task) {
            echo '<div class="task" data-id="' . $task['id'] . '" data-name="' . htmlspecialchars($task['name']) . '" data-description="' . htmlspecialchars($task['description']) . '" data-due_date="' . htmlspecialchars($task['due_date']) . '">'
                ...
                . '<button class="btn btn-link delete-task" data-id="' . $task['id'] . '"><i class="bi bi-trash"></i></button>'
                . '</div>';
        }
    }
    echo '    </div>
                        ...
                        if (confirm("Êtes-vous sûr de vouloir supprimer cette tâche ?")) {
                            ...
                            .then(data => {
                                if (data.success) {
                                    ...
                                    location.reload(); // Recharger la page pour mettre à jour affichage
                                } else {
